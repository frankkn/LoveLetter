...
function saveGame() {
    ...
    writeOfflineSave({
        ...
        recentBaronGuardClue: getRecentBaronGuardClue(),
        championThreshold,
    });
    updateContinueBtn();
}
...
function loadAndStartSave(): boolean {
    ...
    try {
        ...
        setRecentBaronGuardClue(data.recentBaronGuardClue);
...
function render() {
    ...
    opponents.forEach(bot => {
        ...
        // 說話中狀態（WebRTC 說話偵測）
        if (voiceController.isPlayerSpeaking(bot.id)) {
...
function eliminate(
    ...
    clearKnownCardForPlayer(state, playerId);
...
function bindGameRoom(room: Room<unknown, SyncedRoomState>, isReconnect = false) {
    ...
    room.onMessage<{ type: string; existingParticipants?: string[]; sessionId?: string }>(
        'webrtc_voice_state',
        data => voiceController.handleVoiceState(data)
    );

    room.onMessage<{ from: string; type: 'offer' | 'answer' | 'ice'; payload: unknown }>(
        'webrtc_signal',
        data => voiceController.handleSignal(data)
    );
...
const voiceController = createVoiceController({
    getRoom: () => activeGameRoom,
    isOnlineGameActive,
    getPlayerSessionIds: () => currentRoomWaitState?.players.map(p => p.id) ?? [],
});
