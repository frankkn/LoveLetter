...
### v2.12.0 - 代碼審查第三輪：過期重連彈窗、大廳凍結、確認覆蓋與 WebRTC 節流

本版源自第三輪完整代碼審查，共 6 個獨立修正。

**多人連線**

- 修正過期重連 token 造成的假「斷線」彈窗：`bindGameRoom` 在建立／加入房間當下就把重連 token 寫入 sessionStorage，而等待室的「離開房間」不會清除它。之後任何一次頁面載入（含在等待室按 F5）都會跳出注定失敗的重連彈窗——遊戲開始前離開的玩家在伺服器端直接移除、沒有重連視窗。現改為遊戲真正開始（`initOnlineGame`）才寫入，主動離開房間時一併清除。
- 修正大廳連線斷開後房間列表永久凍結：`connectLobbyRoom` 見 `lobbyRoom` 已存在即直接返回，但斷線時沒有任何地方清掉這個失效的 handle，伺服器重啟或網路閃斷後「重新整理」按鈕永久失效，只能整頁重載。現註冊 `onLeave` 清除失效 handle（主動清理會先移除監聽器，不會誤觸發）。
- 修正男爵對決／國王交換的確認被舊同步蓋掉：`applyOnlineGameState` 的保留互動路徑直接以來包覆寫整個 pending 物件，若一包較舊的同步在本地確認送出後才抵達，剛記下的 `confirmedPlayerIds` 會被洗掉，翻牌彈窗會再跳一次要玩家重按確認。同一場對決的確認只增不減，現改為集合聯集合併。

**安全性 / 防護**

- WebRTC 信令補上伺服器端頻率限制：聊天／表情／同步在 v2.11.0 已節流，但 `webrtc_signal` 仍可全速中繼任意 payload 給指定玩家、`webrtc_join_voice` 可無上限觸發全房廣播，是房內騷擾／DoS 向量。沿用既有滑動視窗節流（signal 120／10s、語音加入離開 10／10s），上限遠高於正常流量（對 3 個 peer 的 trickle ICE 尖峰約數十則）。

**音訊**

- 修正切換 BGM 時單次音效未中斷的重疊問題：`playBGM` 只在「同曲目」快速路徑會打斷進行中的勝負 jingle，切到不同曲目（例如 jingle 播放中切換場景）時 jingle 會疊在新 BGM 底下繼續播，且冠軍主題曲播畢的 `onended` 還會把主選單音樂劫持成遊戲 BGM。現在切曲時一律先停掉單次音效並清除其 `onended`。

**維護 / i18n**

- 補齊漏網的硬編碼中文字串：線上房 bot 名稱（單機路徑已用 `player.botA/B/C`，線上獨漏）、語音「說話中」tooltip、聊天室空狀態提示、麥克風權限 alert，英文介面玩家先前會看到中文。新增 `voice.speaking`／`voice.micDenied`／`chat.empty` 三個 key；bot 名稱隨同步狀態送出，各端顯示為房主語言。
- 全部 36 個 E2E 測試通過；前端／後端 tsc 型別檢查與 production build 通過。

**部署提醒**：本版含後端變更（WebRTC 信令節流），需重新部署 Colyseus 伺服器方能完整生效。
